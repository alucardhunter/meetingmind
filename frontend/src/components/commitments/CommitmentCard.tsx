...
const statusConfig = {
  open: { variant: 'warning' as const, icon: Circle, color: 'text-amber-500', labelKey: 'commitments.status.open' },
  fulfilled: { variant: 'success' as const, icon: CheckCircle2, color: 'text-green-500', labelKey: 'commitments.status.fulfilled' },
  overdue: { variant: 'error' as const, icon: AlertTriangle, color: 'text-red-500', labelKey: 'commitments.status.overdue' },
};
...
// Compact variant: used in commitments page list
function CompactCard({
  commitment,
  onToggleStatus,
  onUpdate,
  showMeetingLink,
}: Omit<CommitmentCardProps, 'variant'> & { variant: 'compact' }) {
        ...
        {editingText ? (
          <div className="flex items-start gap-2">
            <textarea
              ...
              rows={2}
            />
            <button onClick={handleTextSave} className="text-green-600 hover:text-green-700 mt-1">
              <Save className="w-4 h-4" />
            </button>
            <button onClick={handleTextCancel} className="text-slate-400 hover:text-slate-600 mt-1">
              <X className="w-4 h-4" />
            </button>
          </div>
        ) : (
...
// Full variant: used in meeting detail page
function FullCard({
  commitment,
  onToggleStatus,
  onUpdate,
  showMeetingLink,
}: Omit<CommitmentCardProps, 'variant'> & { variant: 'full' }) {
  const { t } = useI18n();
  const [editingText, setEditingText] = useState(false);
  const [editTextValue, setEditTextValue] = useState(commitment.text);
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex flex-col items-end gap-2 flex-shrink-0">
            <Badge variant={isOverdue ? 'error' : config.variant}>
              {isOverdue ? t('commitments.status.overdue') : t(config.labelKey)}
            </Badge>
...
// Main exported component with variant routing
export function CommitmentCard(props: CommitmentCardProps) {
  const { variant = 'full' } = props;
  if (variant === 'compact') {
    return <CompactCard {...props} variant="compact" />;
  }
  return <FullCard {...props} variant="full" />;
}
